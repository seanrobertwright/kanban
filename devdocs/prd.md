...
**Acceptance:** ◐ every task mutation (create/update/move/delete/assign/prioritize/schedule/label/comment) writes an `activity_log` row with actor attribution, verified by 111 cases against real Postgres; ✅ history renders on a task. The criterion re-opens as subtasks — the last M1 feature — adds mutations.
**Risk:** low.
...
**Priority and due dates design, as built** (§9's "fields an agent reasons over", less labels):

- **Scope split from labels, because they are not the same shape.** The bullet reads as one item and is three: a priority and a due date are columns on `task`, while labels are a table, a join table, and a vocabulary someone has to manage. Bundling them would have made one diff the size of the previous three M1 commits and hidden a schema decision inside a field addition. Labels land next, workspace-scoped and as a controlled set — an agent choosing from a fixed vocabulary is the difference between `bug` and `bug`/`Bug`/`BUG`, and §5 rules out custom fields anyway.
- **The commit's whole insight is that these two fields sit on opposite sides of 004's `COALESCE` problem, and the rule that decides it generalizes.** Priority is `NOT NULL DEFAULT 'none'`: because `'none'` is a *value* meaning "untriaged", `null` never has to mean "clear", which frees it to mean "not supplied" — so `COALESCE($n, priority)` is correct, exactly as it is for `title`. A due date has no such value; clearing it *is* writing null, so it needs the three-valued supplied-flag `assignee_id` was given at 004. Stated once, for everything after: **a field needs the supplied-flag iff it has no non-null value meaning "empty".** Two fields, one migration, opposite treatment, one rule.
- **`task_priority` is an enum, and the declaration order is the reason.** 001's rule (enumerate closed sets, leave TEXT for what grows) already pointed here, but the deciding fact is that Postgres sorts an enum by declaration order — so `ORDER BY priority DESC` yields urgent→none with no lookup table and no magic integers, and the same array orders the UI and tells the feed whether a priority rose or fell. `SMALLINT` would buy room to interpolate values later, which is precisely the growth a closed set is declaring it does not want. A test asserts the sort, because TEXT would pass every other test in the file and fail only there — alphabetically `urgent` sorts after `none`, so a board ordered by priority would look *almost* right.
- **`due_date` is `DATE`, not `TIMESTAMPTZ`, and this is the decision the field turns on.** A due date is a calendar date, not an instant: "due Friday" does not arrive nine hours early for a teammate in Tokyo. TIMESTAMPTZ would force us to invent a time of day nobody chose, anchor it in a zone somebody has to pick, and answer "which midnight?" forever. DATE has no zone to be wrong about. The cost is that *overdue* needs a zone from outside — which is real, and is paid in the client, against the reader's own local date, the only frame in which the question means anything.
- **The `DATE` type parser is the load-bearing line, and the bug it prevents was reproduced rather than assumed.** node-postgres parses `2026-08-01` into a JS `Date` at *local* midnight. Run in `Asia/Tokyo`, that serializes to `2026-07-31T15:00:00.000Z` — the day before, and from the database's point of view correctly, because the value was never an instant and making it one is what breaks it. West of Greenwich it survives, so this is a bug that passes CI in a UTC container and is wrong for half the planet. `setTypeParser(DATE, v => v)` keeps a date a `'YYYY-MM-DD'` string end to end, where it sorts and compares lexicographically — which covers every question the UI asks of it. Global rather than a `to_char()` per SELECT, so it also covers the queries nobody has written yet.
- **`task.prioritized` and `task.scheduled` are separate actions, and the line that justifies them is now written down.** 004 split `task.assigned` off for three reasons and let the agent-trigger decide it; that leaves open whether every field eventually earns an action, which would make the union the column list spelled twice. The rule: **an action exists when its inverse is something someone would want to apply on its own.** Nobody reverts a title edit but keeps the description edit from the same submit — one authored change, one `task.updated`. But M2's changeset review accepts an agent's proposed actions *in parts*, and criterion #1's triage agent produces exactly "set priority to Urgent" as a unit a reviewer takes or leaves. The due date's case is thinner and worth admitting: no milestone makes it a trigger. It carries on the first clause alone — a date is a commitment rather than content, and "moved the due date to Friday" is what a reader scans for while "updated this task" is what they scroll past.
- **The feed says "raised" and "lowered", not "changed".** Direction is the entire content of a priority change; "changed priority to High" makes the reader find the previous entry to learn whether that is news. This is what `PRIORITY_ORDER`'s ordering buys, and it is why the array exists rather than a set.
- **`TaskSnapshot.priority` is optional but never null, and the asymmetry with `dueDate` is honest.** Both arrived at 006, so pre-006 rows have neither key — 003's rule, that a field skipped on an append-only table is a window lost for good. But the *column* was backfilled to `'none'` truthfully, since nobody could set a priority before it existed, so `undefined` here means only "written before 006" and carries none of `assigneeId`'s ambiguity. `dueDate` keeps all three states. The test that catches this is the `toEqual` in `activity.test.ts` written at 003 to fail whenever the snapshot gains a field — it has now caught 004 and 006, which is two more times than a comment would have.
- **Overdue is decided in the client, and `useToday` is a `useSyncExternalStore` because of it.** The server has no reader and therefore no today: answering in the container's zone would have a card rendered at 8pm in Denver arrive from the server claiming a task due *today* was overdue, then contradict itself on hydration. `getServerSnapshot` returns null — the argument exists for exactly this — so server and first client render agree that nothing is overdue, and the answer arrives on hydration without the extra render pass an effect costs. Three tests render through `renderToString` to prove it; swapping in a naive `new Date()` fails two of them and passes the other six, which is the evidence that those two carry the property.
- **Both signals are stated in words, not only in colour.** The priority dot is the entire signal on a card and roughly one reader in twenty cannot separate the amber from the red, so the label is the content and the dot is its rendering. `'none'` renders nothing at all rather than a grey dot: it is the default state of most cards, and a board of grey dots costs every card the space to say nothing.
- **`<input type="date">` is the rare case where the platform control is exactly right.** Its value is `'YYYY-MM-DD'` whatever locale it displays in — so the string the API wants is the string the DOM already holds, with no parsing, no formatting, and no `Date` to drag a zone through. It clears to `""` on its own, which is the only other state the field has.
- **The date validator round-trips through `Date.UTC`, and the UTC is not confusion about what a date is.** `2026-02-30` matches the shape and is not a date; Postgres would reject it as a 22008 and turn a caller's typo into our 500. Checking with local-time `new Date(2026, 1, 30)` would silently roll to March 2nd *in the server's zone*, making the validity of user input depend on where the container runs. UTC is the one frame that is the same everywhere, which makes it the right one to do zone-free arithmetic in.

**Deliberately not built:** no filtering or sorting by either field, and no priority index. The board reads a column at a time and sorts a handful of rows, which an index cannot beat; the query that would want one — "the backlog, highest priority first" — is M4's, and its shape is unknown enough that an index built today would likely be the wrong one. `due_date` gets a partial index only because M3's calendar view names the query that reads it.

**Labels design, as built** (§9's third triage field, and the one that is not a field):

- **A controlled vocabulary, and that is the entire decision.** A `TEXT[]` on `task` would have been half the code and is what Jira does. But the question the schema has to answer is what an *agent* may write, and M2's criterion #1 is an agent labelling twenty inbound bugs: free text lets it invent `bug`, `Bug`, `bugs` and `defect` across twenty tasks, each a category of one. A closed set makes labelling a **choice from a vocabulary** — a thing a model does well, a human can audit, and M5's "when a bug is labeled P0" can be written against. The unique index on `(workspace_id, lower(name))` *is* that guarantee; everything else is a table of strings.
- **Workspace-scoped, not board-scoped — the one real choice, and the opposite of `board_column`'s.** A workflow belongs to a board: "In Review" means what this board's team does, and another board may not have the step. A vocabulary does not: "bug" means the same thing wherever it is written. Per-board labels would have each board redefine it and leave M3's cross-board filters joining on a string. It also matches where the reader is — an agent is given a workspace, not a board.
- **`labelIds` is two-valued, and 006's rule predicted it without being re-derived.** A set has a non-null value meaning empty — `[]` — so `null` never has to mean "clear", and no supplied-flag is needed. That rule now covers five fields across two migrations, which is the test of whether it was worth writing down. The `COALESCE(json_agg(...), '[]')` in `taskColumns` is what keeps it true: `json_agg` over no rows is `NULL`, and a null leaking through would put the three-valued problem straight back.
- **Two opposite rules apply at once, to different objects, and the split is the clearest one yet.** The *snapshot* carries label **names** — `ColumnSnapshot.title`'s rule, because `task_label` CASCADEs and a deleted label leaves an id pointing at nothing, so the record of a labelling would become unreadable exactly when someone tidies the list. The *task* carries names too, but for a different reason than it might have: `assigneeId`'s rule says carry only an id and resolve client-side, and that would have held here except the log needs the name anyway — so the choice was never "id or name", it was "carry it, or re-fetch it for every log write". Deleting a label five hundred tasks wear makes that concrete: one query, or five hundred. The **colour** stays a client-side lookup, which is where `assigneeId`'s rule does apply.
- **Deleting a label is allowed while tasks wear it — the opposite call from deleting a populated column, and the difference is what the CASCADE destroys.** `task.column_id` CASCADEs to *tasks*, so that button would delete work and is refused with a 409. `task_label` CASCADEs to *links*: the tasks are untouched and lose a label, which is what "delete this label" means. Refusing until every task is unlabelled by hand would be ceremony — the same call the last-column case makes.
- **But it logs one `task.labeled` row per affected task**, which is why it cannot just `DELETE` and let the CASCADE run. `unassignFromWorkspace`'s rule: a reader of a task's history is the only audience for why a label vanished from their card, and one "deleted a label" row in a workspace feed is neither attributable per task nor revertible.
- **A rename logs one row, not five hundred.** It changes what every card says, and the tasks did not change — the vocabulary did. `sameLabels` compares **ids as a set** rather than names, which is what makes that true; comparing names would log a `task.labeled` row per card on every rename, burying the actual event under bookkeeping. The set comparison also survives someone re-ordering the picker's output, where a positional compare would log a change that never happened, permanently, on an append-only table.
- **`labelsSubquery` shipped with a bug worth recording, because it compiles and lies.** `WHERE tl.task_id = id` looks correlated and is not: Postgres resolves an unqualified name against the innermost scope first, and that scope joins `label`, which has an `id`. So it silently asked whether a task id equalled a *label* id — both integers, no error, wrong set. The outer reference has to be spelled `task.id` or `t.id`. A read-back test caught it; the write path could not, because it never re-reads.
- **The picker cannot create a label, and that is the design rather than an omission.** The moment a task dialog can mint one, every hurried edit adds one and the controlled set becomes a suggestion. Labels are managed in their own dialog, against the list you already have. A test asserts the absence, so a well-meaning "just let me type a new one here" has to break it in order to land.
- **`label_color` is an enum for the closed set only, and the emphasis is the opposite of `task_priority`'s.** There the declaration order *is* the sort order and half the reason to use an enum; nothing orders by colour and the order is arbitrary. A palette rather than free hex for the same reason the vocabulary is closed: a board where everyone picks their own `#hex` stops being scannable, and lets someone choose white on white.
- **The trim is correctness, not tidiness.** `lower('bug ')` and `lower('bug')` are different strings, so an untrimmed name walks straight past the one constraint that makes the vocabulary controlled and lands as two labels that render identically — a bug whoever reports it cannot see.
- **The uniqueness check is in the repository *and* the index, and only the index holds.** The check exists so the answer is a sentence naming the existing label in the case it was created with; two concurrent requests can both pass it and both insert, and a test inserts past the repository to prove the index is real.
- **`taskColumns` gained the label subquery rather than each caller gaining a join.** Labels are the first thing to land on `task` after 9edeff3, and a read that forgot them would be typed as a whole `Task` and render cards with none. A correlated subquery is the only shape that fits a column list; a JOIN would multiply the rows and push a `GROUP BY` into every caller, which is exactly the per-caller knowledge that drifts.

**Deliberately not built:** no label filter on the board, and no `task.labeled` for a task created with labels. The filter is M3's view work — there is nothing to filter until boards hold more than a demo's worth of tasks. The log entry is skipped because labels are part of what was *created*, not a change to it: there is no `before` for them to differ from, and undo would invert it to "remove the labels from a task that does not exist".
