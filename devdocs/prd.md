...
- **Labels** — the third, split out below.
...
**Acceptance:** ◐ every task mutation (create/update/move/delete/assign/prioritize/schedule/comment) writes an `activity_log` row with actor attribution, verified by 77 cases against real Postgres; ✅ history renders on a task. The criterion re-opens as each remaining M1 feature adds mutations.
**Risk:** low.
