...
function DocsDialog({ workspaceId, boardId, canManage, open, onOpenChange }: { workspaceId: string; boardId: number; canManage: boolean; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  // Promotes the first unchecked action item. The item comes from the SERVER's
  // extraction of the saved body, not from local editor state — the
  // collaborative editor's in-memory copy can drift from what was saved (OBS-11
  // promoted a title with a heading fused onto it that way). And the error is
  // cleared up front, so a stale failure cannot outlive a later success.
  async function promoteAction() { if (!selected) return; setError(null); try { const [next] = await api.fetchMeetingActions(selected.id); if (!next) { setError("Add an action item like: - [ ] Follow up (and save the document first)"); return; } const response = await fetch(`/api/docs/${selected.id}/promote-action`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: next.title }) }); if (!response.ok) throw new Error((await response.json().catch(() => null))?.error ?? "Could not promote action"); } catch (e) { setError(e instanceof Error ? e.message : "Could not promote action"); } }
