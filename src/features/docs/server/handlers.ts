...
export async function handlePublicDoc(request: Request, token: string) { const limited = publicRateLimit(request, token); if (limited) return limited; try { return Response.json(await getPublicDoc(token), { headers: { "Cache-Control": "no-store" } }); } catch (error) { return authzErrorResponse(error); } }
