...
/** Whether an S3 endpoint is configured at all — the switch storage.ts reads to
 *  decide between this store and the local-disk one. All three are required
 *  together: an endpoint with no credentials is a misconfiguration, not a
 *  reason to silently start writing files to the container's disk. */
