...
export function EnterpriseControls({ workspace }: { workspace: WorkspaceMembership }) {
  ...
  // The starter manifest asks for the narrowest useful grant. The other
  // capabilities (comments.read, labels.read, board.read) are typed in
  // deliberately, which is the point: an owner reads the list before installing.
  ...
  // A download, not a navigation (OBS-13): window.location.assign(exportUrl)
  // replaced the whole app with raw JSON from the SEARCH endpoint — the real
  // export (POST, Content-Disposition: attachment) never ran. Fetch the bundle
  // and hand it to the browser as a file instead.
  async function exportBundle() {
    ...
    anchor.href = href;
    anchor.download = "ediscovery.json";
    anchor.click();
  ...
  return <div className="grid gap-4 border-t pt-3">
    ...
    <section className="grid gap-2"><h3 className="text-sm font-medium">eDiscovery</h3><div className="flex gap-2"><Input aria-label="Search workspace records" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tasks, comments, docs, audit" /><Button size="sm" disabled={busy || !search.trim()} onClick={() => void run(async () => setHits(await api.searchDiscovery(workspace.id, search)))}>Search</Button><Button size="sm" variant="outline" disabled={busy || !search.trim()} onClick={() => void run(exportBundle)}>Export JSON</Button></div><div className="grid gap-1 text-xs">{hits.slice(0, 10).map(hit => <p key={`${hit.subjectType}-${hit.id}`}>{hit.subjectType} #{hit.id}: {hit.title}{hit.onHold && <span className="ml-1 rounded border px-1 text-micro uppercase" title="Under an active legal hold">hold</span>}</p>)}{hits.length > 10 && <p className="text-muted-foreground">Showing 10 of {hits.length}. The export carries them all.</p>}{hits.length === 500 && <p role="status" className="text-destructive">Capped at 500 matches — narrow the search; more records may match.</p>}</div></section>
