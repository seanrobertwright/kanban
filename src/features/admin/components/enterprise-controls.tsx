...
export function EnterpriseControls({ workspace }: { workspace: WorkspaceMembership }) {
  ...
  async function load() {
    const [nextRetention, nextHolds, nextProviders, nextIntegrations] = await Promise.all([
    ...
    setRetention(nextRetention); setHolds(nextHolds); setProviders(nextProviders); setIntegrations(nextIntegrations);
  }
  ...
  return <div className="grid gap-4 border-t pt-3">
    <section className="grid gap-2"><h3 className="text-sm font-medium">Retention</h3><p className="text-xs text-muted-foreground">The scheduled sweep applies activity-log policies. Holds block deletion of tasks, comments, attachments, and documents.</p>{owner && <div className="flex flex-wrap gap-2"><select aria-label="Retention record type" value={retentionSubject} onChange={e => setRetentionSubject(e.target.value as RetentionPolicy["subjectType"])}>{retentionSubjects.map(s => <option key={s}>{s}</option>)}</select><Input aria-label="Retention days" className="w-28" type="number" min="1" value={days} onChange={e => setDays(e.target.value)} /><Button size="sm" disabled={busy || !days} onClick={() => void run(async () => { const maxAgeDays = Number(days); if (!Number.isInteger(maxAgeDays) || maxAgeDays < 1) throw new Error("Retention days must be a positive integer"); await api.saveRetentionPolicy(workspace.id, retentionSubject, maxAgeDays); })}>Save</Button></div>}<div className="grid gap-1 text-xs">{retention.map(p => <p key={p.id}>{p.subjectType}: {p.maxAgeDays} days</p>)}</div></section>
    <section className="grid gap-2"><h3 className="text-sm font-medium">Legal holds</h3>{owner && <div className="flex flex-wrap gap-2"><select aria-label="Hold record type" value={holdSubject} onChange={e => setHoldSubject(e.target.value as LegalHold["subjectType"])}>{holdSubjects.map(s => <option key={s}>{s}</option>)}</select><Input aria-label="Record id" className="w-24" value={holdId} onChange={e => setHoldId(e.target.value)} placeholder="ID" /><Input aria-label="Hold reason" value={holdReason} onChange={e => setHoldReason(e.target.value)} placeholder="Reason" /><Button size="sm" disabled={busy || !holdId || !holdReason} onClick={() => void run(async () => { await api.placeLegalHold(workspace.id, holdSubject, holdId, holdReason); setHoldId(""); setHoldReason(""); })}>Hold</Button></div>}<div className="grid gap-1 text-xs">{holds.map(h => <div className="flex gap-2" key={h.id}><span className="flex-1">{h.subjectType} #{h.subjectId}: {h.reason}</span>{owner && <Button variant="ghost" size="icon" className="size-6" aria-label="Release legal hold" disabled={busy} onClick={() => void run(() => api.releaseLegalHold(workspace.id, h.id))}><Trash2 /></Button>}</div>)}</div></section>
