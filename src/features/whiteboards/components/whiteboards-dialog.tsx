...
function WhiteboardsDialog({ boardId, canEdit, open, onOpenChange }: { boardId: number; canEdit: boolean; open: boolean; onOpenChange: (value: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-6xl"><DialogHeader><DialogTitle>Whiteboards</DialogTitle><DialogDescription>Excalidraw canvas saved to this board. Add task cards, draw, and arrange ideas.</DialogDescription></DialogHeader><div className="grid grid-cols-[11rem_1fr] gap-3"><aside className="grid content-start gap-2 border-r pr-3">{boards.map((board) => <button key={board.id} onClick={() => choose(board)} className={`rounded px-2 py-1 text-left text-sm ${selected?.id === board.id ? "bg-muted font-medium" : "hover:bg-muted"}`}>{board.title}</button>)}{canEdit && <div className="flex gap-1"><Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Board name" /><Button size="sm" onClick={() => void create()}>Add</Button></div>}</aside><section>{selected ? <><div className="mb-2 flex justify-end gap-2">{canEdit && <><Select aria-label="Task to add" className="h-8 rounded bg-background px-2" value={taskId} onValueChange={setTaskId}><SelectItem value="">Add task card…</SelectItem>{taskOptions.map((task) => <SelectItem key={task.id} value={String(task.id)}>#{task.id} {task.title}</SelectItem>)}</Select><Button size="sm" disabled={!taskId} onClick={() => void addTaskCard()}><Plus /> Task card</Button></>}</div><div className="h-[32rem] overflow-hidden rounded border"><Excalidraw key={sceneKey} initialData={{ elements: scene as never[] }} viewModeEnabled={!canEdit} onChange={(elements) => { if (canEdit) handleChange(elements as unknown as Element[]); }} /></div></> : <p className="text-sm text-muted-foreground">Create a whiteboard.</p>}</section></div></DialogContent></Dialog>;
}
